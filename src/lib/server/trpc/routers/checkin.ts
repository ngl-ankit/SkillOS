...
export const checkInRouter = ctx.router({
	today: ctx.protectedProcedure.input(z.object({ day: daySchema.optional() })).query(async ({ ctx: c, input }) => {
		...
		const [existing] = await db.select().from(checkIns).where(and(eq(checkIns.userId, c.user.id), eq(checkIns.day, day))).limit(1);
		const [plan] = await db.select().from(dailyPlans).where(and(eq(dailyPlans.userId, c.user.id), eq(dailyPlans.day, day))).limit(1);
		...
		return {
			...
			plan: plan ? { id: plan.id, focus: plan.focus, budgetMinutes: plan.budgetMinutes, plannedMinutes: plan.plannedMinutes, rationale: plan.rationale } : null,
	...
	history: ctx.protectedProcedure.input(z.object({ limit: z.number().int().min(1).max(60).default(14) })).query(async ({ ctx: c, input }) =>
	...
	pending: ctx.protectedProcedure.query(async ({ ctx: c }) => {
		const day = todayISO();
		const [existing] = await db.select({ id: checkIns.id }).from(checkIns).where(and(eq(checkIns.userId, c.user.id), eq(checkIns.day, day))).limit(1);
