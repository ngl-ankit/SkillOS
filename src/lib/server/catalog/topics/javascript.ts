...
export const JS_TOPICS: CatalogTopic[] = [
	{
		...
		domain: 'JavaScript',
		description:
			'Values, types, variables, operators, conditionals, loops and functions — the building blocks of every JavaScript program.',
		difficulty: 1,
		minutes: 240,
		...
		questions: [
			mcq(
				...
				'c',
				'Primitive types',
				'A long-standing quirk.'
			),
			mcq(
				'Which comparison is true?',
				['0 === "0"', 'null === undefined', '1 === 1.0', '[] === []'],
				'c',
				'Equality (=== vs ==)',
				'1 and 1.0 are the same number; arrays compare by reference.'
			),
			tf(
				'An object in a `const` variable can still have its properties changed.',
				true,
				'Variables (let/const)',
	...
	},
	{
		...
		domain: 'JavaScript',
		description:
			'Model data with arrays and objects, transform it with map/filter/reduce, and destructure and spread values cleanly.',
		difficulty: 2,
		minutes: 180,
		skills: ['javascript'],
		...
		practice: [
			{
				title: 'Shopping cart',
				prompt: 'Given [{name, price, qty}], compute the total and return a discounted copy without mutating the original.'
			},
		...
		questions: [
			mcq(
				'Which method returns a new array of elements passing a test?',
				['map', 'filter', 'forEach', 'find'],
				'b',
				'map/filter/reduce',
				'filter keeps truthy results.'
			),
			mcq(
				'After `const { a, ...rest } = { a: 1, b: 2, c: 3 }`, rest is…',
				['{ a: 1 }', '{ b: 2, c: 3 }', '[2, 3]', 'undefined'],
				'b',
				'Spread and rest',
			...
			code(
				'Return names of users aged 18+ from `users = [{ name, age }]` in one expression.',
	...
	},
	{
		...
		practice: [
			{
				...
				prompt: 'Write makeCounter() returning { increment, decrement, value } sharing private state.'
			},
			{
				title: 'Fix the loop',
				prompt: 'Explain why `for (var i=0;i<3;i++) setTimeout(()=>console.log(i))` prints 3,3,3 and fix it two ways.'
			}
		],
		...
		questions: [
			mcq(
				'What is a closure?',
				['A finished function', 'A function bundled with references to its surrounding scope', 'Closing a tab', 'An IIFE'],
				'b',
				'Closures',
			...
			tf(
				'`let` variables declared in a block are accessible outside it.',
				false,
	...
	},
	{
		...
		domain: 'JavaScript',
		description:
			'Select and update elements, respond to events, and understand bubbling and delegation — making pages interactive.',
		difficulty: 2,
		minutes: 150,
		skills: ['dom'],
		...
		questions: [
			mcq(
				...
				[
					'Browser chooses handlers',
					'One parent listener handling events from many children',
					'One-time listeners',
					'Custom events'
				],
				'b',
			...
			mcq(
				'Safest way to insert user-provided text?',
				['innerHTML', 'textContent', 'document.write', 'outerHTML'],
				'b',
				'Updating the DOM',
				'textContent never parses HTML (prevents XSS).'
			),
	...
	},
	{
		...
		practice: [
			{
				...
				prompt: 'Fetch https://jsonplaceholder.typicode.com/users, render names, and show a friendly error offline.'
			},
		...
		questions: [
			...
			mcq(
				...
				['The raw value', 'A Promise', 'undefined', 'A callback'],
				'b',
				'async/await',
				'Return values are wrapped in a Promise.'
			),
			mcq(
				'Run independent requests concurrently and wait for all with…',
				['await in a loop', 'Promise.all([...])', 'setTimeout', 'Promise.resolve()'],
				'b',
				'Promises',
				'Promise.all starts them together.'
			),
			short(
	...
	},
	{
		...
		questions: [
			mcq(
				'For "^1.4.2", npm may install…',
				['Only 1.4.2', '1.4.x', '1.x.x', 'Any'],
				'c',
				'Semantic versioning',
				'Caret keeps the left-most non-zero digit.'
			),
	...
	},
	{
		...
		practice: [
			{
				title: 'Type an API response',
				prompt: 'Type a response union {status:"ok",data} | {status:"error",message} and handle both.'
			},
		...
		questions: [
			mcq(
				'`string | number` means…',
				['Both', 'Either a string or a number', 'A tuple', 'A generic'],
				'b',
			...
			code(
				'Write a type guard `isString(x: unknown)`.',
				'function isString(x: unknown): x is string { return typeof x === "string"; }',
				['x is string', 'typeof'],
				'Union types and narrowing',
				'`x is string` narrows on true.'
			)
		]
	},
	{
		...
		questions: [
			mcq(
				...
				['Everything on screen', 'Minimal changing data; derive the rest', 'Only server data', 'Nothing'],
				'b',
				'Derived values',
				'Minimal state avoids sync bugs.'
			),
	...
	},
	{
		...
		questions: [
			mcq(
				'Most confidence that a full journey works?',
				['Unit', 'Snapshot', 'End-to-end', 'Type check'],
				'c',
				'End-to-end tests',
				'E2E runs the real app.'
			),
			tf(
				'Good UI tests query by role and label, not CSS classes.',
				true,
	...
	},
	{
		...
		prerequisites: ['components-state'],
		practice: [
			{ title: 'Lighthouse audit', prompt: 'Fix the top two Lighthouse opportunities on a project; record before/after.' }
		],
		...
		questions: [
			...
			tf(
				'Lazy-loading below-the-fold images can speed up initial load.',
				true,
				'Image optimisation',
				'Frees bandwidth for critical assets.'
			),
			short(
