...
export const WEB_TOPICS: CatalogTopic[] = [
	{
		...
		practice: [
			{
				...
				prompt:
					'Open DevTools → Network, load any site and note the method, status code and three response headers of the main document.',
				hint: 'Filter by "Doc" to find the HTML document.'
			},
			{
				title: 'Explain it simply',
				prompt: 'In five sentences, explain to a non-technical friend what happens when they visit a website.'
			}
		],
		...
		questions: [
			mcq(
				'What does DNS do?',
				['Encrypts traffic', 'Translates a domain name into an IP address', 'Renders HTML', 'Stores cookies'],
				'b',
				'DNS resolution',
				'DNS resolves names like example.com to an IP address.'
			),
			mcq(
				...
				'c',
				'Status codes',
				'4xx means the request was wrong; 5xx means the server failed.'
			),
			tf(
				'HTTP is stateless: each request is independent unless state is added, e.g. with cookies.',
				true,
				'HTTP request/response',
				'Sessions are layered on top of HTTP.'
			),
			short(
	...
	},
	{
		...
		description:
			'Navigate the file system, run programs and combine small tools with pipes. The terminal is where builds, Git and deployments live.',
		difficulty: 1,
		minutes: 75,
		...
		practice: [
			{
				title: 'Scaffold from the terminal',
				prompt: 'Create "playground" with src/index.html and README.md using only the terminal, then list the tree.',
				hint: 'mkdir -p, touch, ls -R'
			},
		...
		questions: [
			mcq(
				...
				['Runs commands in parallel', 'Sends output of one command as input to the next', 'Writes to a file', 'Runs on failure'],
				'b',
			...
			tf(
				...
				false,
				'Pipes and redirection',
				'The reverse: > overwrites, >> appends.'
			),
			short(
	...
	},
	{
		...
		domain: 'Developer Tools',
		description:
			'Track changes, work on branches and collaborate through remotes. Git gives you a safety net and a history of every decision.',
		difficulty: 2,
		minutes: 120,
		...
		practice: [
			{
				...
				prompt: 'Change the same line on main and a feature branch, merge, and resolve the conflict.'
			},
		...
		questions: [
			mcq(
				...
				['Creates a commit', 'Stages changes for the next commit', 'Uploads to GitHub', 'Creates a branch'],
				'b',
				'Commits and staging',
			...
			mcq(
				...
				['Different file names', 'Same lines changed differently on both branches', 'Forgetting to pull', 'Empty message'],
				'b',
				'Resolving conflicts',
				'Git cannot choose between two edits of the same lines.'
			),
			short(
	...
	},
	{
		...
		domain: 'Web Foundations',
		description:
			'Structure content with meaningful elements so browsers, search engines and assistive technology understand the page.',
		difficulty: 1,
		minutes: 120,
		skills: ['html'],
		...
		practice: [
			{
				...
				prompt: 'Write a blog post page with header, nav, main, article, aside and footer, one h1 and logical headings.'
			},
			{
				title: 'Accessible form',
				prompt: 'Build a sign-up form with labelled name, email and password fields and a submit button.'
			}
		],
		resources: {
			primary: 'mdn-learn-html',
			alternative: 'webdev-learn-html',
		...
		questions: [
			mcq(
				...
				['<section>', '<div id="content">', '<main>', '<article>'],
				'c',
				'Semantic elements',
				'<main> is the dominant-content landmark.'
			),
			mcq(
				'A control that performs an action without navigating should be…',
				['<a href="#">', '<button>', '<div onclick>', '<span role="link">'],
				'b',
				'Links vs buttons',
				'Buttons act; links navigate.'
			),
			tf(
				'Placeholder text can replace a <label>.',
				false,
				'Forms and labels',
				'Placeholders disappear and are not reliably announced.'
			),
			short(
	...
	},
	{
		...
		domain: 'Web Foundations',
		description:
			'Selectors, the cascade, specificity and the box model — the rules deciding which styles apply and how much space things take.',
		difficulty: 1,
		minutes: 150,
		skills: ['css'],
		...
		practice: [
			{
				title: 'Specificity puzzle',
				prompt: 'Write element, class and id rules for one paragraph and predict which color wins.'
			},
		...
		questions: [
			mcq(
				...
				['p', '.card p', '#intro', 'div p span'],
				'c',
				'Cascade and specificity',
				'IDs outweigh classes and elements.'
			),
			mcq(
				'With `box-sizing: border-box`, width includes…',
				['Content', 'Content + padding', 'Content + padding + border', 'Everything incl. margin'],
				'c',
				'Box model',
	...
	},
	{
		...
		domain: 'Web Foundations',
		description:
			'Arrange elements in one dimension with Flexbox and two with Grid — tools that handle nearly every modern layout.',
		difficulty: 2,
		minutes: 150,
		...
		resources: {
			primary: 'css-tricks-flexbox',
			alternative: 'css-tricks-grid',
			practice: 'flexbox-froggy',
			project: 'frontend-mentor'
		},
		questions: [
			mcq(
				'In a row flex container, which property aligns items vertically?',
				['justify-content', 'align-items', 'flex-direction', 'place-content'],
				'b',
				'Main vs cross axis',
				'align-items works on the cross axis.'
			),
			mcq(
				'`grid-template-columns: 1fr 2fr` produces…',
				['1px and 2px columns', 'Two columns, second twice as wide', 'Two rows', 'Three columns'],
				'b',
				'fr unit',
				'fr shares free space proportionally.'
			),
			tf(
				'Grid suits 2D layouts; Flexbox suits 1D layouts.',
				true,
				'Grid tracks and areas',
	...
	},
	{
		...
		domain: 'Web Foundations',
		description:
			'Make layouts adapt from phones to wide screens with fluid sizing, media queries and responsive images — mobile-first.',
		difficulty: 2,
		minutes: 90,
		skills: ['responsive'],
		...
		questions: [
			mcq(
				'"Mobile-first" CSS means…',
				[
					'Phones only',
					'Base styles for small screens, min-width queries for larger',
					'max-width queries everywhere',
					'A separate mobile site'
				],
				'b',
				'Mobile-first',
				'Start simple, enhance with space.'
			),
			tf(
				'Without the viewport meta tag, mobile browsers may render at desktop width.',
				true,
				'Viewport meta',
				'width=device-width fixes it.'
			),
			code(
				'Write a media query for viewports at least 768px wide.',
				'@media (min-width: 768px) { ... }',
				['@media', 'min-width:\\s*768px'],
				'Media queries',
				'min-width layers larger-screen styles.'
			)
		]
	},
	{
		...
		domain: 'Web Foundations',
		description:
			'Build interfaces everyone can use: keyboard navigation, focus management, contrast, alt text and when (not) to use ARIA.',
		difficulty: 2,
		minutes: 120,
		...
		practice: [
			{
				...
				prompt: 'Navigate a site with Tab, Shift+Tab, Enter and Space only; list where you got stuck.'
			},
		...
		questions: [
			mcq(
				...
				['Always add roles', 'Prefer native HTML elements when one exists', 'aria-label everything', 'ARIA replaces HTML'],
				'b',
				'ARIA roles',
				'Native elements include semantics and keyboard support.'
			),
			mcq(
				'Alt text for a purely decorative image?',
				['alt="image"', 'alt="decorative"', 'alt=""', 'No alt'],
				'c',
				'Alt text',
				'Empty alt tells assistive tech to skip it.'
			),
