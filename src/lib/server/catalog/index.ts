...
export const TOPICS = [...WEB_TOPICS, ...JS_TOPICS, ...BACKEND_TOPICS, ...DATA_AI_TOPICS, ...CS_TOPICS, ...DESIGN_BUSINESS_TOPICS];
...
export function trackOrderIsValid(track: CatalogTrack): boolean {
	...
	return order.every((k, i) => (TOPIC_BY_KEY.get(k)?.prerequisites ?? []).every((p) => (index.get(p) ?? Number.POSITIVE_INFINITY) < i));
}
...
export function validateCatalog(): string[] {
	...
	for (const t of TOPICS) {
		...
		for (const slug of Object.values(t.resources)) if (slug && !RESOURCE_BY_SLUG.has(slug)) problems.push(`${t.key}: unknown resource ${slug}`);
		...
		for (const q of t.questions) {
			...
			if ((q.type === 'short' || q.type === 'code') && !q.keywords?.length) problems.push(`${t.key}: free-text question without keywords`);
